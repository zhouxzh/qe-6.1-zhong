...
END SUBROUTINE selfen_elec_k
  ...
  USE elph2,         ONLY : etf, ibndmin, ibndmax, nkqf, xqf, &
                            nkf, epf17, wkf, nqtotf, wf, wqf, xkf, nkqtotf, &
                            sigmar_all, sigmai_all, zi_all, sigmai_mode, efnew, &
                            sigmar_all_T, sigmai_all_T, zi_all_T, sigmai_mode_T
  USE control_flags, ONLY : iverbosity
  USE constants_epw, ONLY : ryd2mev, one, ryd2ev, two, zero, pi, ci, eps6, kelvin2ev
  USE mp,            ONLY : mp_barrier, mp_sum
  USE mp_global,     ONLY : inter_pool_comm
  !
  implicit none
  ...
  !
  INTEGER :: temp_step
  !! Current temperature-point index 
  !
  ! Local variables 
  CHARACTER (len=256) :: nameF
  !! Name of the file
  ...
  !! Name of the file of the linewidth_elself
  LOGICAL :: opnd
  !! Check whether the file is open. 
  ! 
  INTEGER :: ios
  ...
  ! 
  IF ( iq.eq. 1) THEN
      WRITE(stdout,'(/5x,a,f10.6,a)' ) 'Electron selfen energy calculation with variable temperature = ', eptemp*ryd2ev/kelvin2ev, ' K.'
      WRITE(stdout,'(/5x,a,f10.6,a)' ) 'Electron selfen energy calculation with maximum temperature = ', tempsmax, ' K.'
      WRITE(stdout,'(/5x,a,f10.6,a)' ) 'Electron selfen energy calculation with minimum temperature = ', tempsmin, ' K.'
      WRITE(stdout,'(/5x,a,f10.6,a)' ) 'Electron selfen energy calculation with total temperature steps = ', float(nstemp), ' .'
  ENDIF
  !
  IF ( iq .eq. 1 ) THEN
     !
  ...
  IF (restart) THEN
    ! Make everythin 0 except the range of k-points we are working on
    CALL errore('selfen_elec_q_T', 'The restart function is not implemented.', 1)
  ENDIF
  IF (iq == 1 .and. iverbosity == 3) THEN
    WRITE(stdout,'(5x,"Mode resolved linewidth calculation is performed")')
    ! Make everythin 0 except the range of k-points we are working on
  ENDIF
  !
  IF ( iq .eq. 1 ) THEN 
  ...
  ENDIF
  IF ( iq .eq. 1 ) THEN 
     IF ( .not. ALLOCATED (sigmar_all_T) ) ALLOCATE( sigmar_all_T(ibndmax-ibndmin+1, nksqtotf, nstemp) )
     IF ( .not. ALLOCATED (sigmai_all_T) ) ALLOCATE( sigmai_all_T(ibndmax-ibndmin+1, nksqtotf, nstemp) )
     IF ( .not. ALLOCATED (zi_all_T) )     ALLOCATE( zi_all_T(ibndmax-ibndmin+1, nksqtotf, nstemp) )
     sigmar_all_T(:,:,:) = zero
     sigmai_all_T(:,:,:) = zero
     zi_all_T(:,:,:) = zero
     IF ( iverbosity == 3 ) THEN
       IF ( .not. ALLOCATED (sigmai_mode_T) ) ALLOCATE( sigmai_mode_T(ibndmax-ibndmin+1, nmodes, nksqtotf, nstemp) )
       sigmai_mode_T(:,:,:,:) = zero
     ENDIF
  ENDIF
  !
  ! loop over all k points of the fine mesh
  !
  DO temp_step = 1, nstemp
    eptemp = (tempsmin+(tempsmax-tempsmin)/(nstemp-1)*(temp_step-1))*kelvin2ev/ryd2ev
    inv_eptemp0 = 1.0/eptemp
    inv_degaussw = 1.0/degaussw
    sigmar_all(:,:) = sigmar_all_T(:,:,temp_step)
    sigmai_all(:,:) = sigmai_all_T(:,:,temp_step)
    zi_all(:,:) = zi_all_T(:,:,temp_step)
     IF (iverbosity == 3) sigmai_mode(:,:,:) = sigmai_mode_T(:,:,:,temp_step)
    fermicount = 0 
    DO ik = 1, nkf
        !
        ikk = 2 * ik - 1
    ...
    ENDDO ! end loop on k
    !
    sigmar_all_T(:,:,temp_step) = sigmar_all(:,:)
    sigmai_all_T(:,:,temp_step) = sigmai_all(:,:)
    zi_all_T(:,:,temp_step) = zi_all(:,:)
     IF (iverbosity == 3) THEN
       sigmai_mode_T(:,:,:,temp_step) = sigmai_mode(:,:,:)
     ENDIF
  ENDDO ! end loop on temperature
  !
  !
  ! The k points are distributed among pools: here we collect them
  !
  IF ( iq .eq. nqtotf ) THEN
     ...
     CALL mp_sum( zi_all, inter_pool_comm )
     IF (iverbosity == 3) CALL mp_sum( sigmai_mode, inter_pool_comm )
     CALL mp_sum( sigmar_all_T, inter_pool_comm )
     CALL mp_sum( sigmai_all_T, inter_pool_comm )
     CALL mp_sum( zi_all_T, inter_pool_comm )
     IF (iverbosity == 3) CALL mp_sum( sigmai_mode_T, inter_pool_comm )
     CALL mp_sum(fermicount, inter_pool_comm)
     CALL mp_barrier(inter_pool_comm)
     !
#else
     ...
     DO temp_step = 1, nstemp
        ...
        !
        WRITE(stdout,'(5x,"WARNING: only the eigenstates within the Fermi window are meaningful")')
        WRITE(stdout, '(a, i10, a)') '!    The calculation temperature is ', INT(eptemp*ryd2ev/kelvin2ev), ' K'
        !
        ! Write to file
        WRITE(file_name, '(i10, a)') INT(eptemp*ryd2ev/kelvin2ev), '.linewidth.elself'
        OPEN(unit=linewidth_elself,file=file_name)
        WRITE(linewidth_elself, '(a)') '# Electron lifetime (meV)'
         IF ( iverbosity == 3 ) THEN
           WRITE(linewidth_elself, '(a)') '#      ik       ibnd                 E(ibnd)      imode          Im(Sgima)(meV)     Z'
         ELSE
        WRITE(linewidth_elself, '(a)') '#      ik       ibnd                 E(ibnd)      Im(Sgima)(meV)        Z'
         ENDIF
        ! 
        DO ik = 1, nksqtotf
            !
    ...
    !                             ryd2mev * sigmai_all (ibnd,ik), zi_all (ibnd,ik)
          IF ( iverbosity == 3 ) THEN
            ...
            DO imode=1, nmodes
              ...
              WRITE(linewidth_elself,'(E22.14,2x)',advance='no') ryd2ev * ekk
              WRITE(linewidth_elself,'(i9,2x)',advance='no') imode
              WRITE(linewidth_elself,'(E22.14,2x)', advance='no') ryd2mev*sigmai_mode(ibnd,imode,ik)
              WRITE(linewidth_elself,'(E22.14,2x)') zi_all(ibnd,ik)
            ENDDO
          ELSE
                WRITE(linewidth_elself,'(i9,2x)',advance='no') ik
        ...
        ENDIF
        !
        ENDDO ! End loop over temperature
    ENDIF 
  !
  100 FORMAT(5x,'Gaussian Broadening: ',f10.6,' eV, ngauss=',i4)
  101 FORMAT(5x,'DOS =',f10.6,' states/spin/eV/Unit Cell at Ef=',f10.6,' eV')
  ...
  RETURN
  !
  END SUBROUTINE selfen_elec_q_T
  !
